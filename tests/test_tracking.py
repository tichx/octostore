...
import octostore
from octostore import tracking
from octostore.entities import (
    RunStatus,
    LifecycleStage,
    Metric,
    Param,
...
from octostore.mlflow.protos.databricks_pb2 import ErrorCode, INVALID_PARAMETER_VALUE
from octostore.mlflow_client import mlflow


def test_create_experiment():
    with pytest.raises(TypeError):
        mlflow.create_experiment()  # type: ignore

    with pytest.raises(Exception):
        mlflow.create_experiment(None)  # type: ignore
...
def test_list_experiments():
    def _assert_exps(ids_to_lifecycle_stage, view_type_arg):
        ...
        assert result == set(
            [(exp_id, stage) for exp_id, stage in ids_to_lifecycle_stage.items()]
        )
    ...
    _assert_exps(
        {"0": LifecycleStage.ACTIVE, "1": LifecycleStage.ACTIVE}, ViewType.ACTIVE_ONLY
    )
    ...
    _assert_exps(
        {"0": LifecycleStage.ACTIVE, "1": LifecycleStage.DELETED}, ViewType.ALL
    )
    ...
    reset_mock(
        ...
        mock.Mock(
            return_value=attrdict.AttrDict(
                experiment_id=0, lifecycle_stage=LifecycleStage.ACTIVE
            )
        ),
    )
...
def test_log_batch():
    ...
    assert len(finished_run.data.tags) == len(exact_expected_tags) + len(
...
def test_get_artifact_uri_with_artifact_path_unspecified_returns_artifact_root_dir():
    with mlflow.start_run() as active_run:
        assert (
            mlflow.get_artifact_uri(artifact_path=None) == active_run.info.artifact_uri
        )
...
def test_get_artifact_uri_appends_to_uri_path_component_correctly(
    ...
    client.create_experiment(
        "get-artifact-uri-test", artifact_location=artifact_location
    )
...
def test_search_runs():
    ...
    def verify_runs(runs, expected_set):
        assert set([r.info.run_id for r in runs]) == set(
            [logged_runs[r] for r in expected_set]
        )

    experiment_id = (
        MlflowClient().get_experiment_by_name("exp-for-search").experiment_id
    )
    ...
    runs = MlflowClient().search_runs(
        [experiment_id], "tags.t1 = 'first-tag-val'", ViewType.ALL
    )
    verify_runs(runs, ["first"])
    runs = MlflowClient().search_runs(
        [experiment_id], "tags.t2 != 'qwerty'", ViewType.ALL
    )
    verify_runs(runs, ["second"])
    runs = MlflowClient().search_runs(
        [experiment_id], "tags.t2 = 'second-tag-val'", ViewType.ALL
    )
    verify_runs(runs, ["second"])
    ...
    runs = MlflowClient().search_runs(
        [experiment_id], "params.p1 = 'a'", ViewType.DELETED_ONLY
    )
    verify_runs(runs, ["first"])

    runs = MlflowClient().search_runs(
        [experiment_id], "params.p1 = 'a'", ViewType.ACTIVE_ONLY
    )
    verify_runs(runs, [])
...
def test_search_runs_multiple_experiments():
    experiment_ids = [
        mlflow.create_experiment("exp__{}".format(exp_id)) for exp_id in range(1, 4)
    ]
    ...
    assert (
        len(MlflowClient().search_runs(experiment_ids, "metrics.m0 > 0", ViewType.ALL))
        == 3
    )

    assert (
        len(MlflowClient().search_runs(experiment_ids, "metrics.m_1 > 0", ViewType.ALL))
        == 1
    )
    assert (
        len(MlflowClient().search_runs(experiment_ids, "metrics.m_2 = 2", ViewType.ALL))
        == 1
    )
    assert (
        len(MlflowClient().search_runs(experiment_ids, "metrics.m_3 < 4", ViewType.ALL))
        == 1
    )
